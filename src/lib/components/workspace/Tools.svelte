<script lang="ts">
	...
	const importToolHandler = async (toolData) => {
		try {
			if (toolData) {
				const res = await createNewTool(localStorage.token, toolData).catch((error) => {
					toast.error(`${error}`);
					return null;
				});
			...
			toast.error($i18n.t('Failed to import tool: Invalid tool data'));
			console.error(error);
		}
	};
		...
		<div class=" flex justify-end w-full mb-2">
			<div class="flex space-x-2">
				...
				<input
					...
					on:change={(e) => {
						...
						reader.onload = async (event) => {
							try {
								let savedTool = JSON.parse(event.target.result);
								console.log('Imported tool data:', savedTool);

								// Validate it's a single tool export
								if (Array.isArray(savedTool) && savedTool.length === 1) {
									await importToolHandler(savedTool[0]);
								} else {
									toast.error($i18n.t('Invalid file: Please use a file exported from tool menu'));
								}
							} catch (error) {
								toast.error($i18n.t('Failed to import tool: Invalid file format'));
								console.error(error);
							}
				...
				<button
					class="flex text-xs items-center space-x-1 px-3 py-1.5 rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200 transition"
					on:click={() => {
						const input = document.getElementById('single-tool-import-input');
						input?.click();
					}}
				>
					<div class=" self-center mr-2 font-medium line-clamp-1">{$i18n.t('Import Tool')}</div>
