...
from open_webui.utils.utils import get_verified_user, get_admin_or_content_admin_user
...
async def create_new_knowledge(form_data: KnowledgeForm, user=Depends(get_admin_or_content_admin_user)):
...
async def reset_knowledge_by_id(id: str, user=Depends(get_admin_or_content_admin_user)):
    try:
...
async def delete_knowledge_by_id(id: str, user=Depends(get_admin_or_content_admin_user)):
    try:
        VECTOR_DB_CLIENT.delete_collection(collection_name=id)
    except Exception as e:
        log.debug(e)
